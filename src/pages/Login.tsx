...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate("/"); // Redirect to dashboard or learning page
    } catch (error: any) {
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-background relative overflow-hidden">
      ...
      <div className="hidden lg:flex items-center justify-center relative bg-accent/30 p-12 backdrop-blur-xl border-r border-white/5">
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="max-w-md text-center relative z-10">
          ...
          <p className="text-lg text-muted-foreground leading-relaxed">Continue your learning journey with AI-powered courses and personalized recommendations specifically for your track.</p>
        </motion.div>

        {/* Floating elements inside left panel */}
        <motion.div animate={{ y: [0, 15, 0] }} transition={{ duration: 4, repeat: Infinity }} className="absolute top-[20%] right-[15%] bg-background/80 p-3 rounded-2xl shadow-xl backdrop-blur-md border border-white/10">
           <span className="text-2xl">🎓</span>
        </motion.div>
        <motion.div animate={{ y: [0, -15, 0] }} transition={{ duration: 3.5, repeat: Infinity, delay: 0.5 }} className="absolute bottom-[25%] left-[15%] bg-background/80 p-3 rounded-2xl shadow-xl backdrop-blur-md border border-white/10">
           <span className="text-2xl">📈</span>
        </motion.div>
      </div>
      ...
      <div className="flex items-center justify-center p-8 relative z-10">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} className="w-full max-w-md space-y-8 bg-card/50 p-8 sm:p-10 rounded-3xl shadow-2xl shadow-black/5 ring-1 ring-white/10 backdrop-blur-xl">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-foreground">Log In</h1>
            <p className="text-base text-muted-foreground mt-2">Enter your credentials to continue</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button type="submit" disabled={loading} className="w-full rounded-2xl h-14 text-lg font-bold shadow-lg shadow-primary/25 hover:shadow-primary/40 transition-all">
              {loading ? <Loader2 className="animate-spin w-6 h-6" /> : "Log In securely"}
            </Button>
          ...
          <p className="text-center text-sm font-medium text-muted-foreground pt-4">
            Don't have an account? <Link to="/register" className="text-primary hover:underline underline-offset-4">Create one freely</Link>
          </p>
        </motion.div>
      </div>
    </div>
