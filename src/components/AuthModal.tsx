...
import { X, BookOpen, Loader2, Eye, EyeOff } from "lucide-react";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";
import { useAuthModal } from "@/context/AuthModalContext";

const inputClass =
  "w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-3.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button type="submit" disabled={loading}
        className="w-full rounded-2xl h-12 font-bold text-base bg-primary hover:bg-primary/90 text-white shadow-lg shadow-primary/25 transition-all mt-2">
      ...
      <Button variant="outline" type="button"
        className="w-full rounded-2xl h-12 font-semibold border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors flex items-center justify-center">
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-[201] flex items-center justify-center p-4 pointer-events-none">
            ...
            >
              {/* Header */}
              <div className="relative px-8 pt-8 pb-6 bg-gradient-to-br from-primary to-blue-700">
                {/* Close button */}
